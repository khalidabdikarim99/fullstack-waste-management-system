...
const RecyclerProfile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // ✅ Ensure only recyclers can access
        if (!token || !user || user.role.toLowerCase() !== "recycler") {
          navigate("/recycler/login");
          return;
        }

        const res = await fetch(`${API_URL}/auth/recycler/profile`, {
          method: "GET",
          headers: {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_URL}/auth/recycler/profile/update`, {
        method: "PUT",
        headers: {
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_URL}/auth/recycler/change-password`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
          ...
          <div className="p-6 grid md:grid-cols-2 gap-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">Recyclers transform collected materials into new products, reducing the need for virgin resources.</p>
              </div>
            </div>
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">Create value by processing waste materials into marketable recycled products.</p>
              </div>
            </div>
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="font-semibold text-gray-800">Circular Economy</h3>
                <p className="text-gray-600 mt-1">Complete the waste-to-resource cycle by transforming collected materials into new products.</p>
              </div>
            </div>
            <div className="flex items-start gap-4">
              <div className="bg-amber-100 p-3 rounded-full">
                <BuildIcon className="text-amber-600" />
              </div>
              <div>
                <h3 className="font-semibold text-gray-800">Your Role</h3>
                <p className="text-gray-600 mt-1 capitalize">As a {profile?.role || "recycler"}, you're the final step in creating value from waste materials.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white">
            <h2 className="text-2xl font-bold flex items-center gap-2">
              <AccountCircleIcon className="text-3xl" /> Recycler Profile
            </h2>
